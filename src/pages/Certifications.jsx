...
export default function Certifications() {
  ...
  return <div className="max-w-7xl mx-auto px-4 sm:px-6 py-7 space-y-5">
    <div className="flex items-start justify-between gap-4 flex-wrap"><div><div className="flex items-center gap-3 mb-1"><Network size={22} className="text-blue-400" /><h1 className="text-2xl font-bold text-white">Cybersecurity Certification Map</h1></div><p className="text-slate-500 text-sm max-w-3xl">Explore the cybersecurity certification landscape as an interactive network. Larger nodes contain more certifications. Select a field to rank its credentials.</p></div><div className="flex items-center gap-2 text-xs text-slate-500"><CircleHelp size={14} className="text-slate-600" /><span>{filteredTotal} certifications · {CYBERSECURITY_FIELDS.length} fields</span></div></div>
    ...
    <section className="bg-slate-900/70 border border-slate-800 rounded-2xl p-3 sm:p-5 overflow-x-auto"><div className="min-w-[680px] relative h-[520px] sm:h-[580px] bg-slate-950/70 border border-slate-800/80 rounded-xl overflow-hidden" style={{ backgroundImage: "linear-gradient(rgba(30,64,175,.05) 1px, transparent 1px), linear-gradient(90deg, rgba(30,64,175,.05) 1px, transparent 1px)", backgroundSize: "32px 32px" }}>
      ...
      <button onClick={reset} className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-36 h-36 sm:w-40 sm:h-40 rounded-full border-2 border-blue-400 bg-blue-950/95 flex flex-col items-center justify-center z-20 transition-all hover:scale-105 ${!selectedField ? "ring-2 ring-blue-300/70" : ""}`} style={{ boxShadow: "0 0 5px rgba(96,165,250,1), 0 0 14px rgba(59,130,246,.95), 0 0 28px rgba(37,99,235,.75), inset 0 0 12px rgba(59,130,246,.5)" }} title="Show all certifications"><Award size={25} className="text-blue-200 mb-2" /><span className="text-sm font-semibold text-white">All Certifications</span><span className="text-[10px] text-blue-200 mt-1">{filteredTotal} credentials</span><span className="text-[9px] text-slate-500 mt-2">Click to reset</span></button>
      ...
      <div className="absolute left-3 bottom-3 flex items-center gap-2 text-[10px] text-slate-600 bg-slate-950/80 rounded-lg px-2.5 py-1.5 border border-slate-800"><span className="w-1.5 h-1.5 rounded-full bg-blue-500" />Select a field to explore its certifications</div>
      <div className="absolute right-3 bottom-3 z-30 text-[10px] leading-relaxed text-slate-400">
        <div className={`relative w-40 overflow-hidden rounded-lg border bg-slate-950/90 shadow-xl transition-all duration-500 ease-in-out ${showKey ? "max-h-40 translate-y-0 border-slate-700 opacity-100" : "max-h-0 translate-y-full border-transparent opacity-0"}`}>
          <div className="px-3 py-2 font-semibold uppercase tracking-widest text-slate-300">Key</div>
          <div className="overflow-hidden"><div className="space-y-1 border-t border-slate-800 px-3 py-2.5"><div><span className="text-white">Bright node</span> = selected field</div><div><span className="text-slate-300">Soft line</span> = related field</div><div><span className="text-slate-600">Dark node</span> = unrelated field</div></div></div>
        </div>
        <button onClick={() => setShowKey(value => !value)} aria-expanded={showKey} aria-label={showKey ? "Collapse key" : "Expand key"} className="absolute -top-4 right-3 z-10 flex h-6 w-6 items-center justify-center rounded-full border border-slate-700 bg-slate-950 text-blue-300 shadow-[0_0_10px_rgba(59,130,246,.65)] transition-all duration-500 ease-in-out hover:border-blue-500/70 hover:bg-blue-950 hover:text-white">

...
function getCollisionFreePositions(fields, maxCount, viewportWidth) {
  const width = Math.max(viewportWidth - 48, 680);
  const height = viewportWidth < 640 ? 520 : 580;
  const nodes = fields.map((field, index) => ({
    x: DEFAULT_NODE_POSITIONS[index][0],
    y: DEFAULT_NODE_POSITIONS[index][1],
    size: getNodeSize(getFieldCertifications(field.id).length, maxCount),
  }));
...
export default function Certifications() {
  ...
  const maxCount = Math.max(...CYBERSECURITY_FIELDS.map(field => getFieldCertifications(field.id).length), 1);
  const nodePositions = useMemo(() => getCollisionFreePositions(CYBERSECURITY_FIELDS, maxCount, viewportWidth), [maxCount, viewportWidth]);
  ...
  return <div className="max-w-7xl mx-auto px-4 sm:px-6 py-7 space-y-5">
    <div className="flex items-start justify-between gap-4 flex-wrap"><div><div className="flex items-center gap-3 mb-1"><Network size={22} className="text-cyan-400" /><h1 className="text-2xl font-bold text-white">Cybersecurity Certification Map</h1></div><p className="text-slate-500 text-sm max-w-3xl">Explore the cybersecurity certification landscape as an interactive network. Larger nodes contain more certifications. Select a field to rank its credentials.</p></div><div className="flex items-center gap-2 text-xs text-slate-500"><CircleHelp size={14} className="text-slate-600" /><span>{CERTIFICATIONS.length} certifications · {CYBERSECURITY_FIELDS.length} fields</span></div></div>
    ...
    <section className="bg-slate-900/70 border border-slate-800 rounded-2xl p-3 sm:p-5 overflow-x-auto"><div className="min-w-[680px] relative h-[520px] sm:h-[580px] bg-slate-950/70 border border-slate-800/80 rounded-xl overflow-hidden" style={{ backgroundImage: "linear-gradient(rgba(34,211,238,.035) 1px, transparent 1px), linear-gradient(90deg, rgba(34,211,238,.035) 1px, transparent 1px)", backgroundSize: "32px 32px" }}>
      ...
      <div className="absolute right-3 bottom-3 z-30 w-52 rounded-lg border border-slate-700 bg-slate-950/95 px-3 py-2.5 text-[10px] leading-relaxed text-slate-400 shadow-xl"><div className="mb-1 font-semibold uppercase tracking-widest text-slate-300">Key</div><div><span className="text-white">Bright node</span> = selected field</div><div><span className="text-slate-300">Soft line</span> = related field</div><div><span className="text-slate-600">Dimmed node</span> = unrelated field</div></div>
    </div></section>
